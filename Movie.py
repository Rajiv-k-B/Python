...
movies = ['anand', 'drishyam', 'nayakan', 'anbe sivam','gol maal','vikram vedha', 'black fryday', 'dangal', 'manichitranthazhu', 'tarre zameen par' ];
...
def play():
    ...
    while willing:
        if turn%2==0:
            ...
            while not_said:
                letter=input('Your Letter: ')
                if(is_present(letter,picked_movie, letter)):
                    #unlock
                    modified_qn=unlock(modified_qn,picked_movie,letter)
                    print(modified_qn)
                    d=input('press 1 to guess the movie or 2 to unlock another letter: ')
